...
class _GridAttentionBlockND(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels=None, dimension=3, mode='concatenation',
        ...
        assert dimension in [2, 3] #Make sure it is one of the listed dimensions ie. Either 2 or 3.
        assert mode in ['concatenation', 'concatenation_debug', 'concatenation_residual'] #See "assert dimension" comment - MUST be one of the listed values.
        ...
        if mode == 'concatenation':
            self.operation_function = self._concatenation #Unless specified otherwise, the standard operation will be the basic concatenation.
...
"""
############################################
### This is the one we are acctually using. We call it in sononet_grid_attention.py as AttentionBlock2D. Changes:
...
class _GridAttentionBlockND_TORR(nn.Module):
    ...
    def _concatenation(self, x, g):
        ...
        if self.mode == 'concatenation_softmax':
            ...
            psi_f_flat = psi_f.view(batch_size, 1, -1)
            ss = psi_f_flat.shape
            psi_f_min = psi_f_flat.min( )[0].view(ss[0],ss[1],1)
...
###
# Specific calls of the above with certain conditions being set
