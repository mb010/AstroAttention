...
class playground_v4(nn.Module):
    def __init__(self):
        super(playground_v4,self).__init__()
        self.conv1a = nn.Conv2d(in_channels=1,out_channels=6,kernel_size=(3,3),padding=1,stride=1)
        self.conv1b = nn.Conv2d(in_channels=6,out_channels=6,kernel_size=(3,3),padding=1,stride=1)
        self.conv2a = nn.Conv2d(in_channels=6,out_channels=16,kernel_size=(3,3),padding=1,stride=1)
        self.conv2b = nn.Conv2d(in_channels=16,out_channels=16,kernel_size=(3,3),padding=1,stride=1)
        self.conv3a = nn.Conv2d(in_channels=16,out_channels=32,kernel_size=(3,3),padding=1,stride=1)
        self.conv3b = nn.Conv2d(in_channels=32,out_channels=32,kernel_size=(3,3),padding=1,stride=1)
        self.conv4a = nn.Conv2d(in_channels=32,out_channels=64,kernel_size=(3,3),padding=1,stride=1)
        self.conv4b = nn.Conv2d(in_channels=64,out_channels=64,kernel_size=(3,3),padding=1,stride=1)
        ...
        self.fc1 = nn.Linear(64*9*9,2) #channel_size * width * height
        ...
        self.compatibility_score1 = _GridAttentionBlock2D_TORR(in_channels=32 , gating_channels=64, inter_channels=32, use_W=False)
        self.compatibility_score2 = _GridAttentionBlock2D_TORR(in_channels=64, gating_channels=64, inter_channels=32, use_W=False)
    ...
    def forward(self,inputs):
        ...
        pooled = self.dropout(F.adaptive_avg_pool2d(conv5, (1, 1)).view(batch_size, -1))
        ...
        # Aggregation
        g1 = self.dropout(torch.sum(attendedConv3.view(batch_size, self.filters[2], -1), dim=-1))
        g2 = self.dropout(torch.sum(attendedConv4.view(batch_size, self.filters[3], -1), dim=-1))
