...
class AGSononet(nn.Module):
    ...
    def forward(self,inputs):
        ...
        conv1  = self.dropout(F.relu(self.bnorm1(self.conv1a(inputs))))
        conv1  = self.dropout(F.relu(self.bnorm1(self.conv1b(conv1))))
        conv1  = self.dropout(F.relu(self.bnorm1(self.conv1b(conv1))))
        #conv1 = self.dropout(conv1)
        mpool = self.mpool(conv1)
        
        # unetConv2(3) call ([-1,16,75,75]->[-1,32,75,75]) followed by an mpool
        conv2  = self.dropout(F.relu(self.bnorm2(self.conv2a(mpool))))
        conv2  = self.dropout(F.relu(self.bnorm2(self.conv2b(conv2))))
        conv2  = self.dropout(F.relu(self.bnorm2(self.conv2b(conv2))))
        #conv1 = self.dropout(conv2)
        mpool = self.mpool(conv2)
        
        # unetConv2(3) call ([-1,32,37,37]->[-1,64,37,37]) followed by an mpool
        conv3  = self.dropout(F.relu(self.bnorm3(self.conv3a(mpool))))
        conv3  = self.dropout(F.relu(self.bnorm3(self.conv3b(conv3))))
        conv3  = self.dropout(F.relu(self.bnorm3(self.conv3b(conv3))))
        #conv1 = self.dropout(conv3)
        mpool = self.mpool(conv3)
        
        # unetConv2(2) call ([-1,64,18,18]->[-1,128,18,18]) followed by an mpool
        conv4  = self.dropout(F.relu(self.bnorm4(self.conv4a(mpool))))
        conv4  = self.dropout(F.relu(self.bnorm4(self.conv4b(conv4))))
        #conv1 = self.dropout(conv4)
        mpool = self.mpool(conv4)
        
        # unetConv2(2) call ([-1,128,9,9]->[-1,128,9,9])
        conv5 = self.dropout(F.relu(self.bnorm4(self.conv4b(mpool))))
        conv5 = self.dropout(F.relu(self.bnorm4(self.conv4b(conv5))))
        #conv1 = self.dropout(conv5)
