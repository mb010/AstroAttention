...
class AGRadGalNet(nn.Module):
    def __init__(self, aggregation_mode='deep_sup', n_classes=2, AG=3, normalisation='range_norm'):
        ...
        self.conv1a = nn.Conv2d(in_channels=1, out_channels=6, kernel_size=3, padding=1, stride=1); self.bnorm1a= nn.BatchNorm2d(6); self.relu1a = nn.ReLU()
        self.conv1b = nn.Conv2d(in_channels=6, out_channels=6, kernel_size=3, padding=1, stride=1); self.bnorm1b= nn.BatchNorm2d(6); self.relu1b = nn.ReLU()
        self.conv1c = nn.Conv2d(in_channels=6, out_channels=6, kernel_size=3, padding=1, stride=1); self.bnorm1c= nn.BatchNorm2d(6); self.relu1c = nn.ReLU()
        self.mpool1 = nn.MaxPool2d(kernel_size=(2,2), stride=2)
        
        self.conv2a = nn.Conv2d(in_channels=6, out_channels=16, kernel_size=3, padding=1, stride=1); self.bnorm2a= nn.BatchNorm2d(16); self.relu2a = nn.ReLU()
        self.conv2b = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, padding=1, stride=1); self.bnorm2b= nn.BatchNorm2d(16); self.relu2b = nn.ReLU()
        self.conv2c = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, padding=1, stride=1); self.bnorm2c= nn.BatchNorm2d(16); self.relu2c = nn.ReLU()
        self.mpool2 = nn.MaxPool2d(kernel_size=(2,2), stride=2)
        
        self.conv3a = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, padding=1, stride=1); self.bnorm3a= nn.BatchNorm2d(32); self.relu3a = nn.ReLU()
        self.conv3b = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1); self.bnorm3b= nn.BatchNorm2d(32); self.relu3b = nn.ReLU()
        self.conv3c = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, padding=1, stride=1); self.bnorm3c= nn.BatchNorm2d(32); self.relu3c = nn.ReLU()
        self.mpool3 = nn.MaxPool2d(kernel_size=(2,2), stride=2)
        
        self.conv4a = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1, stride=1); self.bnorm4a= nn.BatchNorm2d(64); self.relu4a = nn.ReLU()
        self.conv4b = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1, stride=1); self.bnorm4b= nn.BatchNorm2d(64); self.relu4b = nn.ReLU()
        ...
        ### FIX THIS!!! BNORM AND RELU ARE IN INCORRECT ORDER !!!
    ...
    def forward(self,inputs):
        conv1a = self.relu1a(self.bnorm1a(self.conv1a(inputs))) #1->6
        conv1b = self.relu1b(self.bnorm1b(self.conv1b(conv1a))) #6->6
        conv1c = self.relu1c(self.bnorm1c(self.conv1c(conv1b))) #6->6
        mpool1 = self.mpool1(conv1c)
        
        conv2a = self.relu2a(self.bnorm2a(self.conv2a(mpool1))) #6->16
        conv2b = self.relu2b(self.bnorm2b(self.conv2b(conv2a))) #16->16
        conv2c = self.relu2c(self.bnorm2c(self.conv2c(conv2b))) #16->16
        mpool2 = self.mpool2(conv2c)
        
        conv3a = self.relu3a(self.bnorm3a(self.conv3a(mpool2))) #16->32
        conv3b = self.relu3b(self.bnorm3b(self.conv3b(conv3a))) #32->32
        conv3c = self.relu3c(self.bnorm3c(self.conv3c(conv3b))) #32->32
        mpool3 = self.mpool3(conv3c)
        
        conv4a = self.relu4a(self.bnorm4a(self.conv4a(mpool3))) #32->64
        conv4b = self.relu4b(self.bnorm4b(self.conv4b(conv4a))) #64->64
