...
def show_cam_on_image(img, mask):
    ...
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)#mask cmap.
    heatmap = np.float32(heatmap) / 255#normalise
    cam = heatmap + np.float32(img)#sum
    cam = cam / np.max(cam)#renormalise
    cv2.imwrite("cam.jpg", np.uint8(255 * cam))#Scale for colour output and save
