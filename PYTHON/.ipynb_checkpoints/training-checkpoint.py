...
def train(date,
    ...
    all_labels = [*range(10)]
    if datasubset == 'NOUNC':
        allowed_labels = all_labels[0:3]+all_labels[6:8]
    elif datasubset == 'NOHYBRID':
        allowed_labels = all_labels[0:6]+all_labels[6:8]
    elif datasubset == 'HYBRID':
        allowed_labels = all_labels
    else:
        allowed_labels = all_labels

    date_label = f'{date}-{dataset}{datasubset}{optim_label}' #format: mmdd
    ckpt_name = f"{net_name}-{date_label}-{learning_rate}_{Epoch}Epochs.pt"
    folder_name = f"TrainedNetworks/{date_label}-{net_name}-{learning_rate}"
